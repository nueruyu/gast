...
namespace Gast.Infrastructure.AI.Tools
{
    public class ReflectionToolRegistry : IToolRegistry
    {
        ...
        {
            var keysToRemove = tools.Where(kvp => kvp.Value.Target == target).Select(kvp => kvp.Key).ToList();
        ...
        public async Task<string> ExecuteAsync(string functionName, string argumentsJson)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    if (argsDict != null && argsDict.TryGetValue(snakeName, out var val))
                    {
                        // Convert JSON object to actual parameter type
                        string valJson = JsonConvert.SerializeObject(val);
                    ...
                    {
                        throw new ArgumentException($"Missing required argument: {snakeName}");
                    }
                }
